...
OUTPUT_FILE = os.path.join(BASE_DIR, "AI_GENERATED_TEXT_REPORT.md")

with open(INPUT_FILE, "r", encoding="utf-8") as f:
    data = json.load(f)

total_items = len(data)
q_items = [d for d in data if d.get("metrics", {}).get("candidate_count") == 0]
cand_items = [d for d in data if d.get("metrics", {}).get("candidate_count") == 2]

titles = []
descriptions = []
for d in cand_items:
    for c in d.get("metrics", {}).get("candidates", []):
        titles.append((d["id"], d["name"], c["title"], len(c["title"].split())))
        descriptions.append((d["id"], d["name"], c["description"], len(c["description"].split()), c.get("suggested_root"), c.get("suggested_group")))

title_lengths = [t[3] for t in titles]
desc_lengths = [d[3] for d in descriptions]
title_len_dist = Counter(title_lengths)
desc_len_dist = Counter(desc_lengths)

desc_texts = [d[2] for d in descriptions]
symbols = {
    "Semicolons (;)": sum(1 for t in desc_texts if ";" in t),
    "Ampersands (&)": sum(1 for t in desc_texts if "&" in t),
    "Hyphens / Dashes (-)": sum(1 for t in desc_texts if "-" in t),
    "Slashes (/)": sum(1 for t in desc_texts if "/" in t),
    "Arrows": sum(1 for t in desc_texts if any(a in t for a in ["→", "↑", "↓"])),
    "Abbreviations (w/)": sum(1 for t in desc_texts if "w/" in t)
}
...
for sym, count in symbols.items():
    ...
    role = "Separating distinct petitionary clauses without conjunctions" if ";" in sym else (
           "Joining paired virtues or related concerns (e.g., 'Wisdom & Peace')" if "&" in sym else (
           "Telegraphic sub-clauses and compound terms" if "-" in sym else (
           "Alternative aspects or paired options" if "/" in sym else (
           "Indicating progression or directional outcomes" if "Arrow" in sym else "Compact relational shorthand"))))
    lines.append(f"| **`{sym}`** | {count} ({pct:.1f}%) | {role} |")
...
for w in sorted(title_len_dist.keys()):
    c = title_len_dist[w]
    pct = c / len(titles) * 100
    bar = "#" * int(pct / 2)
    lines.append(f"  {w} words  |  {c:3d}  |   {pct:5.1f}%   | {bar}")

lines.extend([
    "---------------------------------------------------------",
    f"  Total    |  {len(titles)}  |   100.0%   | Mean: {sum(title_lengths)/len(title_lengths):.2f} words",
    "```",
    "",
...
for w in sorted(desc_len_dist.keys()):
    c = desc_len_dist[w]
    pct = c / len(descriptions) * 100
    bar = "#" * int(pct)
    lines.append(f"  {w:2d} words |  {c:3d}  |   {pct:5.1f}%   | {bar}")

lines.extend([
    "---------------------------------------------------------",
    f"  Total    |  {len(descriptions)}  |   100.0%   | Mean: {sum(desc_lengths)/len(desc_lengths):.2f} words (Min: {min(desc_lengths)}, Max: {max(desc_lengths)})",
    "```",
    "",
...
for q in questions:
    inp = q['input'].replace("\n", " ")
    if len(inp) > 40: inp = inp[:37] + "..."
    posture = "Gentle pastoral focus without prying" if "010" in q['id'] else (
              "Evangelistic specificity" if "016" in q['id'] else (
              "Subtle redirection from universe to God" if "026" in q['id'] else (
              "Clarifying decree into genuine need" if "031" in q['id'] else (
              "Subtle redirection from saint to God" if "032" in q['id'] else (
              "Probing heart fear beneath bargaining" if "040" in q['id'] else (
              "Deconstructing transactional barter" if "043" in q['id'] else (
              "Entity context clarification (self vs other)" if "046" in q['id'] else (
              "Burden triage between competing crises" if "061" in q['id'] else (
              "Specific struggle prioritization" if "077" in q['id'] else (
              "Ongoing spiritual thanksgiving focus" if "096" in q['id'] else "Eliciting petition from liturgical adoration"))))))))))
    lines.append(f"| **{q['id']}** | {q['wordiness']} | *\"{inp}\"* | \"{q['question']}\" | {posture} |")
...
theological_terms = {
    ...
    "grace (alone / unmerited)": ["grace"],
    "Christ / Jesus": ["christ", "jesus"],
    "providence / providential": ["providence", "providential"],
    "repentance / repent": ["repentance", "repent"],
    "steadfast / endurance": ["steadfast", "endurance"],
    "contentment / provision": ["contentment", "provision"],
    "wisdom / discernment": ["wisdom", "discernment"],
    "comfort / peace": ["comfort", "peace"],
    "Soli Deo Gloria / glory": ["gloria", "glory"]
}

for family, tokens in theological_terms.items():
    occurrences = sum(1 for d in desc_texts if any(token in d.lower() for token in tokens))
    sig = "Upholding God's sovereign governance over outcomes" if "sovereign" in family else (
          "Affirming Sola Gratia against human merit" if "grace" in family else (
          "Solus Christus: Mediation solely through Christ" if "Christ" in family else (
          "Trusting divine ordering in suffering & life events" if "providence" in family else (
          "Orienting lost and believer petitions toward heart turning" if "repentance" in family else (
          "Historic Reformed posture in trials and spiritual warfare" if "steadfast" in family else (
          "Refusal of greed & prosperity dogmas" if "contentment" in family else (
          "Sound pastoral and ecclesiastical navigation" if "wisdom" in family else (
          "Heidelberg Q1 grounding in affliction" if "comfort" in family else "Ultimate end of all prayer (Article I)"))))))))
    lines.append(f"| **`{family}`** | {occurrences} cards | {sig} |")
...
print(f"Report generated successfully: {OUTPUT_FILE} ({len(lines)} lines)")
