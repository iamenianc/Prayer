...
- **Client Configuration & Preferences Schema (`APP_CONFIG`)**:
  - Encrypted key-value or single-row table in SQLite storing local preferences:
    ...
    - `theme_mode`: `"WARM_VINTAGE_WHITE"` (Single unified theme based around warm vintage whites).
...
- **Sequestered Settings Architecture (`APP_CONFIG`)**:
  ...
  - All configurable parameters (`locale_dialect`, `text_scale`, `blend_historic_prayers`) are isolated inside a dedicated Settings panel reached exclusively via deliberate navigation from the Journal.
- **Data Model to Devotional UI Terminology Mapping Standard**:
  ...
  - `APP_CONFIG` parameters: Rendered respectively as **`Text Size`**, **`Language`**, and **`Historic Prayers`**.
