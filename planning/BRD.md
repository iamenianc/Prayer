...
1. **The Modern Folio Frontispiece & Zero-Flash Launch (`HomeScreen`)**:
   ...
   - Presents strictly three contiguous tactile bookplate actions: elevated sanctuary gateway (*Start praying*), directory records (*Open Journal*), and direct capture notepad (*Add prayer points*).
...
2. **The Four Foundational Roots (`People`, `Groups`, `General`, `Mission Partners`)**:
   ...
   - **`General`**: Broad topics, global matters, societal needs, and historic liturgical prayers. **Each preloaded historic prayer occupies its own distinct entity (topic) within General** — never grouped under a single aggregate entity. This individual-entity structure honors the theological weight and character of each historic prayer. Current preloaded historic topics: The Lord's Prayer, 1662 BCP Collects (Peace, Grace, Purity, General Thanksgiving), The Apostles' Creed, and 8 C.H. Spurgeon Metropolitan Tabernacle pulpit prayers.
...
3. **Contemplative, Passive Prayer Flow (`PrayerSessionScreen`)**:
   ...
   - **Buttonless Navigation & Ergonomic Zones**:
     ...
     - **Center 40% Width**: Reading canvas; long-pressing any prayer point opens an in-place status resolution dialog (`Active` $\leftrightarrow$ `Answered`).
   ...
   - **Read-Only AI Prompts on Past Points (Grouped into Praise God, Thank God, Ask God)**: Displays 3–12 read-only prompts (4–15 words each, at least one per group) categorized under *Praise God*, *Thank God*, and *Ask God*, opening with *For*, *That*, *A* (or *An*), or *Because*, written in warm, flowing, non-terse language without parroting user input verbatim and strictly avoiding wishy-washy general platitudes. Crucially, the **default view is hidden or collapsed**; prompts are surfaced only if the user explicitly taps to view them, preserving quiet contemplative stillness. Every prompt must be a 100% complete, fully finished grammatical thought that never cuts off mid-sentence or ends on dangling connectors. Prompts are freshly derived without overfitting or copying few-shot examples. **AI prompt generation is suppressed entirely for preloaded historic entities** — these entities carry the prayers of the saints and require no AI augmentation.
   ...
   - **Zero-State Fallback (Historic Reformed Prayers)**: When no personal prayer topics exist, the contemplative queue draws from all 14 preloaded historic prayer entities (The Lord's Prayer, five 1662 BCP Collects/Creed, and eight Spurgeon Tabernacle prayers), each surfaced as individual topics.
...
4. **Direct Lined Notepad & Capture Flow (`LogPrayerScreen`)**:
   ...
   - **Auto-Bullets**: Enter/Return inserts `\n• ` seated precisely on the next dynamic baseline rule.
   - **Strictly No AI in Add Mode**: Zero AI assistance, zero suggestions pane, and zero AI interaction while drafting.
   - **Target Selection & Single-Tap Save**: Tapping **Next** transitions to the categorization sheet presenting four spheres (`People`, `Groups`, `General`, `Mission Partners`). Tapping an entity commits immediately and navigates to that entity's view in the Journal.
   - **Undo Toast Feedback**: Immediate Snackbar toast (*"Saved to [Name]"*) with an Undo action that restores the draft.
...
5. **The Modern Folio Design System & The Singular Folio Law**:
   ...
   - **Interactive Silk Marker Ribbon Tab**: Vector swallow-tail notched tab (`18dp` width, `40dp` resting / `54dp` pinned, spring stiffness `220`, damping `0.70`, haptic tick, expanded `48 × 56dp` touch envelope).
...
6. **Journal Directory & In-Place Editing (`JournalScreen`)**:
   - Four collapsible accordion categories (`People`, `Groups`, `General`, `Mission Partners`).
   - Direct addition route: An action placed at the top of each listing under each group (`+ Add person`, `+ Add group`, `+ Add topic`, `+ Add mission partner`) opening an in-place creation dialog and navigating immediately to the subject's record upon save.
...
graph TD
    subgraph Mobile Client (Offline-First Modern Folio)
        ...
        JournalUI["JournalScreen (People | Groups | General | Mission Partners)"]
    ...
    subgraph Free AI Bridge (Zero Auth / Zero Cost)
        ...
        LLM["OpenRouter Inference Gateway<br/>(meta-llama/llama-3.3-70b-instruct, Two-Tier Pipeline)"]
    end
