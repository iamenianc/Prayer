...
Deliver an intimate, distraction-free, mobile-first prayer companion (*Pray Without Ceasing*) that cleanly separates the contemplative act of prayer from the administrative task of organizing prayer points. The application opens to a pristine, blank canvas offering strictly three choices—**Start praying**, **Open Journal**, and **Add prayer points**—grounded in historic Reformed theology and supported by an ambient, non-conversational background suggestion engine provided free of charge.
...
- **Relational Prayer Intercessors**: Believers actively holding prayer points for specific individuals, family members, ministry groups, and global concerns.
...
2. **The Four Foundational Roots (`People`, `Groups`, `General`, `Mission Partners`)**:
   - **`People`**: Exclusively and strictly specific, distinct individual human relationships (e.g., spouse, parent, child, a single named friend/neighbor, and personal prayer points under *Me*—including personal health, job trials, or sanctification situated within a workplace, school, or hospital). Plural or collective peer environments (such as coworkers or classmates as a unit) belong under `Groups`.
   ...
   - **`Mission Partners`**: Supported missionary families, mission agencies, missionaries, and ministry partners (e.g., a missionary family on the field, a Bible translation agency, a church planting ministry).
...
3. **Contemplative, Passive Prayer Flow ("Start Praying" / Full-Screen Prayer Mode)**:
   ...
   - **Expandable Answered Prayer Points**: Answered prayer points for the active topic are rendered softly beneath active prayer points without boxes or tiles, allowing believers to reflect on God's past faithfulness and offer thanksgiving without cluttering active intercession.
...
4. **Entity-First Prayer Adding ("Add Prayer Points")**:
   - **Mandatory Initial Step (Select or Create Target Entity)**:
     - Tapping *Add prayer points* immediately presents an entity selection screen. Every prayer point must belong to a specific person, group, general topic, or mission partner.
     - Users select an existing entity (`People`, `Groups`, `General`, `Mission Partners`) from the local SQLite vault, or tap the contiguous **"New Person / Group"** creation tile to immediately establish a new name and root.
     - When adding is triggered from within an existing entity detail screen in the Journal, the target entity is already locked and this initial step is seamlessly satisfied.
   - **Integrated Lined Notepad (Direct Entry Replacement, Auto-Bullets & Post-Commit Auto-Titling)**:
     - Selecting a person or group immediately opens a dedicated **lined notepad** for writing prayer points directly, completely removing the intermediary "Direct Entry" button.
     - **Authentic Notepad Ruled Lines**: The notepad renders subtle, light horizontal lines across the entire writing area mirroring a physical notebook or legal pad, adapting seamlessly across Morning Light and Quiet Night modes.
     - The user **shall not be able to see or add a title** when adding new points. The view provides strictly an unadorned ruled text canvas pre-bound to the selected person or group.
     ...
     - **Immediate Local Save**: Tapping **Save to [Name]** (or the keyboard-accessible Save action in the top header) immediately commits the prayer point body to the local SQLite database.
     - Following committal, a concise 2–6 word prayer point title is auto-generated asynchronously via a dedicated lightweight branch of the AI engine.
     ...
     - **Offline Graceful Fallback**: In offline scenarios, the prayer point is safely saved locally using an initial truncated text snippet until background connectivity generates the permanent title.
   - **Ambient AI Suggestion Pane (Bottom Collapsible Pane, Batch Context & Scroll-Refresh)**:
     - **Non-Conversational Paradigm**: The AI assistant will **not** ask questions and will **not** be able to chat to. Conversational turn-taking, chat dialogues, and inquiry prompts are completely eliminated.
     - **Background Ambient Operation**: Operates silently in the background to show suggested prayer points based strictly on content already available for a particular prayer target (entity).
     - **Collapsible Bottom Pane**: Formatted as a small pane anchored at the bottom of the screen, which can be **tapped to hide or show** on demand, keeping the primary visual focus on the lined notepad.
     - **Scrollable Suggestions (1 to 5 Lines Visible)**: Houses a scrollable list displaying **1 to 5 lines at a time**.
     - **Line Brevity Standard (1 to 6 Words)**: Each suggested line is strictly between **1 and 6 words long**, providing punchy, scannable prayer points rather than verbose paragraphs.
     - **Strict Non-Fabrication Invariant**: The lines produced by the AI shall **never make up content if existing data is limited**. The engine is strictly bounded by user-supplied facts and must never extrapolate, invent crises, or fabricate medical or emotional circumstances.
     - **Batch Target Context**: All user recorded data on the target (existing active prayer points, answered points, background descriptions, and journal updates) is passed to the AI **in one go, not line by line**, giving the model full holistic context in a single request.
     - **Scroll-Triggered Refresh**: Scrolling within the suggestion list automatically **triggers a call to refresh the list**, fetching fresh suggested lines dynamically.
     - **Single-Tap Adoption**: Tapping any suggested line immediately appends or inserts it into the active lined notepad draft.
...
6. **Granular Prayer Point Lifecycle, Entity Management & Long-Press Responsive Actions**:
   ...
   - **Long-Press Responsiveness & Planar Contextual Menus**:
     ...
     - *Sanctuary Prayer Mode Responsiveness*: Prayer points displayed in full-screen Sanctuary mode are responsive to touch and long-press. Long-pressing a prayer card surfaces an uncluttered, high-contrast modal allowing immediate status toggle without leaving contemplative focus.
...
11. **Universal LIFO Back Stack & Context Preservation**:
   ...
   - Back navigation faithfully unwinds nested context layer by layer (e.g., adding a prayer point from an entity detail screen returns directly to that entity's detail view in the Journal, rather than abruptly collapsing to the root Home menu).
...
- **Non-Conversational & Ambient AI**: The AI assistant will **not** ask questions and will **not** be able to chat to. Conversational turn-taking, artificial empathy, clarifying questions, and chat interfaces are strictly forbidden. The AI works quietly in the background to show suggested prayer points in a bottom pane based on existing content.
- **Objective Prayer Points, Never Scripted Prayers**: The suggestion engine must never compose actual prayers or address God directly (e.g., never output "Father God...", "Dear Lord...", "Lord Jesus...", "Thy will be done", or second-person invocations to God). Believers pray themselves; the engine strictly summarizes the burden or thanksgiving into concise suggested prayer points.
- **Ambient Output Format & Brevity Ceilings**: Suggestions are displayed as a scrollable list showing **1 to 5 lines at a time**, with each line strictly between **1 and 6 words long**. Redundant prefixes (e.g., "Pray for...", "Ask God to...") are stripped.
...
- **Scroll-Triggered Refresh Invariant**: Scrolling the suggestion list automatically **triggers a call to refresh the list**, fetching additional or updated suggestions in the background.
...
graph TD
    subgraph Mobile Client (Offline-First)
        ...
        AddingUI["Add Flow (Lined Notepad + Ambient Bottom Pane)"]
    ...
    AddingUI -.->"Batch Target Context (In One Go)"| Proxy
    Proxy --> LLM
```
