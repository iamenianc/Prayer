...
4. **Entity-First Prayer Adding ("Add Prayer Points")**:
   ...
   - **Integrated Lined Notepad (Direct Entry Replacement, Auto-Bullets & Post-Commit Auto-Titling)**:
     ...
     - **Pushed-Down Secondary Action**: The notepad expands vertically to occupy the vast majority of the viewport, pushing the secondary assisted articulation button (**"Prayer Assistant"**) down to an accessible action slab at the bottom of the screen.
   ...
   - **Prayer Assistant (Assisted Articulation & Distillation)**: Pushed to the bottom of the screen as an objective, structured secondary tool to help users articulate tangled or heavy thoughts for the selected person or group:
...
- **Telegraphic AI Output Convention & Strict Length Ceilings**: AI suggestions employ a compact shorthand tailored for mobile prayer cards:
  ...
  - *High-Level Structure Direction (Embrace Variety)*: Descriptions remain telegraphic and glanceable on mobile, but are explicitly NOT forced into any fixed clause template or ordering. Clause count, punctuation (semicolons, em-dashes, commas), and the sequencing of circumstance, heart posture, and trust in God's sovereign care vary naturally to fit each burden, and the two cards in a response must differ in structure and vocabulary.
  - *Single Focus*: Each card focuses on a single specific burden rather than compounding multiple disparate prayer points.
...
- **Solemn Devotional Motion Mechanics**: Interactions avoid flashy consumer animations in favor of reverent, intentional motion reflecting Anglican prayer traditions: staggered home launch entrance, simulated prayer book page-turning in Sanctuary mode via directional `AnimatedContent` with `FastOutSlowInEasing` (350ms), shared-axis sub-screen slide-and-fade transitions, dynamic list item animations (`Modifier.animateItem()`), and staggered materialization of candidate prayer cards.
...
- **Two-Tier LLM Architecture (Thoughtful Generation + Strict Compliance Harness)**: To resolve the inherent tension between evocative, non-robotic pastoral text and unyielding compliance with theological guardrails, mobile brevity ceilings, and schema invariants, the API proxy deploys a two-tier pipeline in `api/worker.js`. Tier 1 operates at higher temperature (`0.7`, `max_tokens: 9000`) to generate thoughtful, natural, non-generic drafts for clarifying questions, candidate points, and titles without robotic repetitions. Tier 2 operates at low temperature (`0.1`, `max_tokens: 9000`) as a strict verification and compliance harness that rewrites, purifies, and condenses the Tier 1 draft, unconditionally enforcing zero direct address to God, telegraphic scannable description structure, 2–6 word titles, $\le$ 20–25 word descriptions, pre-specified root invariants, and valid JSON wire format.
...
graph TD
    subgraph Mobile Client (Offline-First)
        ...
        AddingUI["Add Flow (Write Point | 'Prayer Assistant')"]
    ...
    AddingUI -.->"Encrypted Payload (Sanitized)"| Proxy
    Proxy --> LLM
```
