...
3. **Contemplative, Passive Prayer Flow ("Start Praying")**:
   ...
   - **Card Focus with Configurable Batches (Contiguous Tessellated Tiles)**: Displays prayer points as completely flat rectangular tiles with sharp 90-degree edges (strictly zero curved edges or rounded corners). Displayed one card at a time by default, with user settings to show 1, 2, 3, 4, or 5 items per view tessellated contiguously with zero visible gaps between adjacent tiles.
