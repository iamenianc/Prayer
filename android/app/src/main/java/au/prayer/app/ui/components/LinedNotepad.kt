...
fun LinedNotepad(
    ...
    if (autoFocus) {
        LaunchedEffect(Unit) {
            // Instantly ensure the typing cursor is positioned at the end of the last line
            if (text.selection.end != text.text.length || text.selection.start != text.text.length) {
                onTextChange(text.copy(selection = TextRange(text.text.length)))
            }
            repeat(5) {
                try {
        ...
        Box(
            ...
            BasicTextField(
                ...
                modifier = Modifier
                    ...
                    .doubleTapWordSelection(
                        getText = { text },
                        onTextChange = onTextChange,
                        textLayoutResult = { textLayoutResult },
                        contentPaddingStart = with(density) { PrayerSpacing.textInset.toPx() },
                        contentPaddingTop = topMarginPx
                    )
                    .padding(
                        top = topMargin,
